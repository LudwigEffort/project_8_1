...
+ [ ] Auth by token in Lab Methods
+ [ ] Token expires
+ [ ] Add communication between api
...
*** Autorizzazioni
L'autorizzazione nel Lab Web API avviene tramite il ~[FromHeader]~ delle richieste, gli passo l'email (con il token per verificare l'autorizzazone), con l'email trovo l'utente nel db e se ha i giusti permessi esegure le operazioni.
